...
export default function InstructorCoursesPanel({ user }) {
  ...
  const fetchCourses = () => {
    ...
    }).then((res) => setCourses(res.data.filter((c) => c.created_by === user.id)));
  };
  ...
  return (
    <div className="page-panel" style={{ flex: 1 }}>
      <h2 style={{ color: 'var(--forest-deep)', marginBottom: '24px' }}>My Courses</h2>
      ...
      <h3 style={{ fontSize: '1rem', marginBottom: '14px', color: 'var(--forest-deep)' }}>Courses You Created</h3>
      <div style={{ display: 'grid', gap: '12px' }}>
        {courses.length === 0 && (
          <p style={{ color: 'var(--muted-light)', fontSize: '0.9rem' }}>You haven't created any courses yet.</p>
        )}
        {courses.map((c) => (
